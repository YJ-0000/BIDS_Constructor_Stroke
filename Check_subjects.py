...
def check_folder_name(subject_number, expected_code, folder_name):
    ...
    # pattern = re.compile(rf"(?i)(?:^|[^0-9]){subject_number}[-_]?{expected_code}")
...
def main(bids_dir):
    ...
    for sub_dir in subject_dirs:
        ...
        if not tsv_files:
            ...
            continue
        # 만약 여러 개라면 첫 번째 파일을 사용 (필요에 따라 수정 가능)
        ...
        try:
            with tsv_file.open(newline='', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    # ntpath.basename을 사용하면 Windows 스타일 경로에서도 마지막 폴더명을 추출할 수 있습니다.
                    ...
                    if check_folder_name(subject_number, expected_code, folder_basename):
                        success_msgs.append(f"{sub_dir.name} - {session_id}: '{folder_basename}' → OK")
                    else:
                        error_msgs.append(
